...
#Mail Headings
    ...
    msg.as_string(a)
#pdf attachment
    Certificate_doc = 'Certificates/{}.pdf'.format(j['Club & Chapter Name'] + '/' + j['Rename Certificate']+j['Name of Winner']) 
    pdf = MIMEApplication(open(Certificate_doc, 'rb').read())
    pdf.add_header('Content-Disposition','attachment',filename=Certificate_doc)
    msg.attach(pdf)
    print(j['sl.no'], 'Mailing Certificate of', j['Name of Winner'])
#Email Settings
    try:
